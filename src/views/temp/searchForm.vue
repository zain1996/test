  ...
  <div class="form-box searchForm">
    ...
    <el-row :span="24">
      ...
      <div class="input-box-center">
        <el-form :inline="true" :model="formData" class="demo-form-inline" v-if="formModel.length>0" label-width="100px"
                 ref="formData">
          <el-row :span="24">
            <div v-for="(model,index) in formModel" :key="model.key" v-if="show ? index !== '': index<=6 " class="item">
              <el-form-item v-if="!model.isGroup" :label="model.name" :key="model.key" :prop="model.key"
              ...
              <el-form-item v-else :label="model.name" :key="model.key" :prop="model.key"
                            :label-width="model.labelWidth">
                <div class="searchMoreWarp">
                  <div class="searchMoreItem" v-for="(v,i) in model.children">
                    <el-input v-if=" v.elemType === 'input' "
                              size="small"
                              :style={width:v.width}
                              v-model="formData[v.key]"
                              :maxLength="v.maxLength"
                              @focus="event => onFocus(event, v, i)"
                              @blur="event => onBlur(event, v, i)"
                              @change="event => onChange(event, v, i)"
                              :placeholder="v.placeholder || ('请输入'+v.name)"
                              clearable>
                    </el-input>
                    <el-select v-else-if=" v.elemType === 'select' "
                               clearable
                               :style={width:v.width}
                               size="small"
                               @focus="event => onFocus(event, v, i)"
                               ...
                               @change="event => onChange(event, v, i)"
                               v-model="formData[model.key]"
                               :placeholder="v.placeholder || ('请选择'+v.name)"
                               :prop="v.key">
                      <el-option v-for=" (opt, index) in v.child "
                                 :value="opt[v.colValue]"
                                 :label="opt[v.colName]"
                                 :key="index">
                      </el-option>
                    </el-select>
            ...
            <el-col :span="6" class="btn">
              <el-form-item>
                <el-button type="primary" size="small" @click="searchBtn()" id="searchBtn" class="btn-active"
                           v-if="searchShow">查询
                </el-button>
                <el-button type="primary" size="small" @click="reset('formData')" id="reset" class="black-button"
                           v-if="resetShow">重置
                </el-button>
                <el-button type="primary" size="small" @click="more()" id="more" class="black-button" v-if="moreShow"
                           :disabled="disMore">更多
