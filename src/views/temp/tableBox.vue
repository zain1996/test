  ...
  <div class="table-box">
            ...
            <el-form-item>
              <div class="btn-left">
                <template v-for="btn in leftButton">
                  ...
                  <el-upload
                    ...
                    :action="btn.action">
                    <el-button v-show="btn.showButton" size="small" type="primary" id="importBtn">{{btn.value}}
                    </el-button>
                  </el-upload>
                </template>
              </div>
              ...
              <div class="btn-right">
                <template v-for="btn in rightButton">
                  <el-button v-if=" btn.elemType === 'output' && !userButtonCodesData[btn.id]" :type="btn.type"
                             :size="btn.size" @click="btn.method()" :id="btn.id" v-text="btn.value"
    ...
    >
      ...
      <template v-for="(col, index) in formDataLabel" v-if="tableShow.indexOf(col.label)>-1">
        <el-table-column v-if="col.linkType" align="center" :type="col.type" :label="col.label" :width="col.width"
                         :min-width="col.minWidth" :key="col.prop">
        ...
        <el-table-column align="center" v-else :type="col.type" :prop="col.prop" :label="col.label" :width="col.width"
                         :min-width="col.minWidth" :key="col.prop" :formatter="col.formatter">
        </el-table-column>
      </template>
      <!--</template>-->
    </el-table>
    <el-row :span="24" v-if="paginationShow">
      <div class="box-page">
        <refresh-icon :tableKey="tableKey" @tableKey="tableKey = arguments[0]"
