<template>
  <div class="login-container">
    <div id="loginBox">
      ...
      <div class="box-right">
        <div class="loginLogo">
          <img src="../../assets/images/login/BanBan-logo.svg" width="100%" height="100%"/>
        </div>
        <div>
                                    ...
                                    if (res.data.status) {
                                        ...
                                        this.$store.dispatch('GenerateRoutes', res.data.data.userInfo).then(() => {
                                            ...
                                            for (let j = 0; j < addRouters.length; j++) {
                                                if (addRouters[j].path === "/system") {
                                                    for (let i = 0; i < addRouters[j].children.length; i++) {
                                                        if (addRouters[j].children[i].path === 'userManagement') {
                                                            this.$router.push({path: '/system/userManagement'});
                                                            break
                                                        } else {
